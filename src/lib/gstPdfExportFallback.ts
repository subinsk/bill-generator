...
export function exportGSTBillToPDFFallback(bill: GSTBill) {
  // This method captures the exact UI appearance using html2canvas
  // It will look exactly like what the user sees on screen
  ...
  console.log('✅ Bill element found, dimensions:', {
    offsetWidth: billElement.offsetWidth,
    offsetHeight: billElement.offsetHeight,
    scrollWidth: billElement.scrollWidth,
    scrollHeight: billElement.scrollHeight,
    clientWidth: billElement.clientWidth,
    clientHeight: billElement.clientHeight
  });
  ...
  tempStyle.textContent = `
    ...
    /* Ensure table fits within PDF width */
    .pdf-export-mode {
  ...
  // Temporarily constrain the element width for PDF capture
  const originalWidth = billElement.style.width;
  const originalMaxWidth = billElement.style.maxWidth;
  // Set a maximum width that will fit in A4 portrait (210mm ≈ 794px at 96dpi)
  billElement.style.width = '750px'; // Slightly less than A4 width to ensure fit
  billElement.style.maxWidth = '750px';
  
  console.log('📏 Element width adjustments:', {
    originalWidth,
    originalMaxWidth,
    newWidth: billElement.style.width,
    newMaxWidth: billElement.style.maxWidth
  });
  
  // Force a reflow to ensure styles are applied
  billElement.offsetHeight;
  
  console.log('📐 After width adjustment:', {
    offsetWidth: billElement.offsetWidth,
    scrollWidth: billElement.scrollWidth,
    clientWidth: billElement.clientWidth
  });
  ...
  const canvasWidth = Math.min(billElement.scrollWidth, 750); // Constrain to PDF width
  const canvasHeight = billElement.scrollHeight;
  const windowWidth = Math.max(window.innerWidth, 800); // Ensure adequate window width
  
  console.log('📸 html2canvas settings:', {
    scale: 1.5,
    canvasWidth,
    canvasHeight,
    windowWidth,
    windowHeight: window.innerHeight,
    elementScrollWidth: billElement.scrollWidth,
    elementScrollHeight: billElement.scrollHeight
  });
  ...
  html2canvas(billElement, {
    ...
    width: canvasWidth, // Use scrollWidth instead of offsetWidth
    height: canvasHeight,
    scrollX: 0,
    scrollY: 0,
    windowWidth: windowWidth, // Extra padding
    windowHeight: window.innerHeight,
    ...
    console.log('✅ Canvas captured successfully!', {
      ...
      actualWidth: canvas.width / 1.5, // Actual width without scale
      actualHeight: canvas.height / 1.5 // Actual height without scale
    });
    ...
    if (pdfButton) {
      ...
      pdfButton.innerHTML = '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg> PDF में एक्सपोर्ट करें';
    }
    ...
    const imgWidth = pageWidth - 10; // Increased margin for better fit
    ...
    if (finalImgHeight <= pageHeight - 10) {
      pdf.addImage(imgData, 'PNG', 5, 5, finalImgWidth, finalImgHeight);
      ...
      for (let i = 0; i < totalPages; i++) {
        ...
        if (pageCtx) {
          ...
          pdf.addImage(pageImgData, 'PNG', 5, 5, finalImgWidth, pageImgHeight);
        }
      }
    }
    ...
    // Clean up temporary style and class on error
    const tempStyle = document.getElementById('pdf-export-override');
    if (tempStyle) {
      tempStyle.remove();
    }
    
    // Remove PDF export class on error
    billElement.classList.remove('pdf-export-mode');

    // Restore original width settings on error
    billElement.style.width = originalWidth;
    billElement.style.maxWidth = originalMaxWidth;
    ...
    toast.error('PDF जेनरेट करने में त्रुटि। कृपया पुनः प्रयास करें।');
  });
}
